defmodule Chirp.Accounts do
  ...
  """
  def update_user_email(user, token, user_token \\ nil) do
    ...
    with {:ok, query} <-
      ...
      AppCache.del_user_by_token(user_token)
      :ok
    else
      _ -> :error
    end
  ...
  """
  def update_user_password(user, password, attrs, user_token \\ nil) do
    changeset =
      user
  ...
  def get_user_by_session_token(token) do
    ...
    {_, user} =
      AppCache.fetch_user_by_token(token, fn ->
        {:ok, query} = UserToken.verify_session_token_query(token)
        user = Repo.one(query, caller: caller)
        {:commit, user}
      end)
  ...
  def delete_session_token(token) do
    ...
    AppCache.del_user_by_token(token)
    :ok
  end
  ...
  """
  def confirm_user(token, user_token \\ nil) do
